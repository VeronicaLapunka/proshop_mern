...
import * as userActions from '../../frontend/src/actions/userActions'

jest.mock('axios')
jest.mock('../../frontend/src/actions/userActions')

const middlewares = [thunk]
const mockStore = configureMockStore(middlewares)

describe('payOrder action creator', () => {
  afterEach(() => {
    jest.clearAllMocks()
  })
  ...
  describe('API errors with response.data.message', () => {
    ...
    // This asserts current buggy behavior. Correct would be: should not logout on substring match
  ...
  describe('API errors without response.data.message', () => {
    ...
    it('crashes when accessing undefined response.data.message (buggy error extraction)', async () => {
      ...
      // This asserts current buggy behavior. Correct would be: check if error.response.data exists before accessing .message
      axios.put.mockRejectedValue({
        response: { status: 500 },
        message: 'Server error',
        // response exists but response.data is undefined — code tries to access response.data.message and crashes
      })
      ...
      try {
        await store.dispatch(payOrder(orderId, paymentResult))
        // If we get here, error handling caught it somehow
        fail('Expected dispatch to throw or dispatch FAIL action')
      } catch (e) {
        // Code crashes trying to read .message on undefined response.data
        expect(e.message).toContain("Cannot read")
      }
    })

    // This asserts current buggy behavior. Correct would be: should not logout without exact message match
  ...
  describe('missing/null inputs', () => {
    ...
    // This asserts current buggy behavior. Correct would be: validate userInfo exists before accessing .token
    it('dispatches REQUEST and FAIL when userInfo is missing', async () => {
      ...
      const actions = store.getActions()
      expect(actions[0]).toEqual({ type: ORDER_PAY_REQUEST })
      // Error is caught and dispatched as FAIL
      expect(actions[1].type).toBe(ORDER_PAY_FAIL)
      expect(actions[1].payload).toContain("Cannot read")
    })

    // This asserts current buggy behavior. Correct would be: validate userLogin exists
  ...
  describe('edge cases with paymentResult', () => {
    ...
    // This asserts current buggy behavior. Correct would be: validate paymentResult shape before sending
  ...
  describe('token in Authorization header', () => {
    ...
    it('sends Authorization: Bearer undefined when token is missing from userInfo', async () => {
      ...
      const callConfig = axios.put.mock.calls[0][2]
      // This asserts current buggy behavior. Correct would be: validate token exists before using it
