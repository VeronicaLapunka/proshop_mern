...
import { z } from "zod";
import { toText, listFeatures, getFeatureInfo, setFeatureState, adjustTrafficRollout, } from "./helpers.js";
...
`.trim(),
    inputSchema: {
        feature_id: z
            .string()
            .describe("snake_case key exactly as it appears in features.json, e.g. 'dark_mode'. Case-sensitive."),
    },
    annotations: { readOnlyHint: true, idempotentHint: true },
...
`.trim(),
    inputSchema: {
        ...
        state: z
            ...
            .describe("Target status — MUST be exactly one of: 'Disabled', 'Testing', 'Enabled' (case-sensitive)."),
    },
