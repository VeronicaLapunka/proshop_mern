...
export const VALID_STATES = ["Disabled", "Testing", "Enabled"] as const;
...
export type FeaturesFile = Record<string, FeatureRecord>;

// ---------------------------------------------------------------------------
// I/O helpers
// ---------------------------------------------------------------------------

export function readFeatures(): FeaturesFile {
  let raw: string;
  try {
...
export function writeFeatures(features: FeaturesFile): void {
...
export function toText(obj: unknown): McpTextContent {
 ...
 */
export function listFeatures(): object {
 ...
 */
export function getFeatureInfo(feature_id: string): object {
  let features: FeaturesFile;
  try {
    features = readFeatures();
  } catch (e: unknown) {
  ...
  const dependency_states: Record<string, string> = {};
  for (const dep of feat.dependencies ?? []) {
    if (features[dep]) dependency_states[dep] = features[dep].status;
  }
 ...
 */
export function setFeatureState(feature_id: string, state: string): object {
  ...
  if (state === "Enabled") {
    ...
    if (disabledDeps.length > 0) {
      return errObj(
        ...
        `Cannot enable '${feature_id}': ${disabledDeps.map((d) => `'${d}'`).join(", ")} ` +
          `is Disabled. Enable the dependency first.`,
        feature_id,
      );
    }
    ...
    for (const dep of deps) {
      if (features[dep]?.status !== "Enabled") {
        warnings.push(
          `Dependency '${dep}' is '${features[dep]?.status}', not 'Enabled'. ` +
 ...
 */
export function adjustTrafficRollout(feature_id: string, percentage: number): object {
  if (!Number.isInteger(percentage) || percentage < 0 || percentage > 100) {
    return errObj(
      "INVALID_PERCENTAGE",
      `percentage must be a whole number between 0 and 100. Got: ${percentage}.`,
      feature_id,
    );
  }
  ...
  if (feat.status !== "Testing") {
    return errObj(
      ...
      `adjust_traffic_rollout can only be called on features with status 'Testing'. ` +
        `'${feature_id}' is currently '${feat.status}'. Use set_feature_state to change its status first.`,
      feature_id,
    );
  }
