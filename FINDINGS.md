...
| 4 | 🔴 CRITICAL | `frontend/src/store.js:68–74` | Redux `initialState` restores `cartItems` and `shippingAddress` from localStorage but omits `paymentMethod`; users lose payment choice on page refresh during checkout flow | Add payment method restoration: `const paymentMethodFromStorage = localStorage.getItem('paymentMethod') ? JSON.parse(localStorage.getItem('paymentMethod')) : null` and include in initial `cart` state | ✅ Fixed in commit b697dcd |
